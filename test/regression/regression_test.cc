...
std::unique_ptr<RegressionTest> RegressionTest::from_yajl_node(const yajl_val &node) {
    ...
    for (int i = 0; i < nelem; i++) {
        ...
        set_opt_int_from_yajl(u->intervention_log_enabled,
            "intervention_log_enabled", key, val);
        set_string_from_yajl(u->intervention_log_api,
            "intervention_log_api", key, val);
...
void RegressionTest::update_expected_from_yajl_node(const yajl_val &val) {
    for (int j = 0; j < val->u.object.len; j++) {
        ...
        set_opt_int_from_yajl(intervention_log_present,
            "intervention_log_present", key2, val2);
        set_string_from_yajl(intervention_log,
            "intervention_log", key2, val2);
...
std::string RegressionTests::toJSON() const {
    ...
    for (const auto &t : tests) {
        ...
        gen_key_opt_int(g, "intervention_log_enabled",
            t->intervention_log_enabled);
        gen_key_str_if_non_empty(g, "intervention_log_api",
            t->intervention_log_api);
        ...
        gen_key_opt_int(g, "intervention_log_present",
            t->intervention_log_present);
        gen_key_str_if_non_empty(g, "intervention_log",
            t->intervention_log);
